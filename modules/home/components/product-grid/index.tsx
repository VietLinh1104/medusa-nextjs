...
const ProductGrid = ({ title, subtitle }: { title: string, subtitle: string }) => {
  const dummyProducts = [
    ...
    { id: "2", name: "Manuka Honey MGO 250+", price: "$65.00" },
        ...
        <div className="grid grid-cols-1 gap-y-12 gap-x-8 sm:grid-cols-2 lg:grid-cols-4">
          {dummyProducts.map((product) => (
             <div key={product.id} className="text-center">
                <ProductCard {...product} />
                <div className="mt-4">
                   <button className="text-[10px] font-bold uppercase tracking-widest text-[#e85a31] border-b border-[#e85a31] hover:text-black hover:border-black transition-colors">
                      Thêm vào giỏ
                   </button>
                </div>
             </div>
