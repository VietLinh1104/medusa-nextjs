...
const Hero = () => {
  ...
  return (
    <div className="relative h-[705px] overflow-hidden group">
      {/* Slides Container */}
      <div
          ...
          <div key={index} className="min-w-full h-full relative flex items-center">
            ...
            <div className="relative max-w-7xl mx-auto px-4 py-32 sm:px-6 lg:px-8 w-full z-10">
              <div className="max-w-2xl">
                ...
                <h1 className="text-4xl font-black tracking-tight text-white sm:text-6xl uppercase leading-tight animate-fade-in-up">
                  {slide.title} <br />
                  <span className="text-[#d4b068]">{slide.subtitle}</span>
                </h1>

                <p className="mt-4 text-lg text-gray-300 font-medium">{slide.desc}</p>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-8 top-1/2 -translate-y-1/2 z-20 w-10 h-10 flex items-center justify-center bg-white hover:bg-[#e85a31] rounded-sm shadow-lg text-[#e85a31] hover:text-white opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0"
      >
      ...
      <button
        onClick={nextSlide}
        className="absolute right-8 top-1/2 -translate-y-1/2 z-20 w-10 h-10 flex items-center justify-center  bg-white hover:bg-[#e85a31] rounded-sm shadow-lg text-[#e85a31] hover:text-white opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-x-2 group-hover:translate-x-0"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
