  ...
  const signals = [
    {
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v13m0-13l-4-4m4 4l4-4M12 11l-3 3m3-3l3 3m-3 3l-2.5 2.5m2.5-2.5l2.5 2.5" />
        </svg>
      ),
    ...
    <div className="grid grid-cols-2 gap-4 py-6 border-t border-gray-100">
      {signals.map((signal, index) => (
        <div key={index} className="flex items-center gap-3">
          ...
          <span className="text-[12px] text-gray-700 leading-tight">{signal.text}</span>
        </div>
      ))}
    </div>
  )
